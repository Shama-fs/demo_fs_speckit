...
def _fs_run(cmd: list[str]) -> str:
    return subprocess.check_output(cmd, text=True, stderr=subprocess.STDOUT).strip()

def _fs_techstack_preflight(require_frontend: bool = True, require_vscode: bool = False) -> None:
    """
    FutureStandard tech stack validation (Python-only).
    Runs during `specify init` BEFORE templates are downloaded/extracted.
    """
    errors: list[str] = []
    ...
    if not py:
        errors.append("Python not found. Install Python 3.10+ (recommended 3.11+).")
    else:
        vtxt = _fs_run([py, "--version"])  # e.g., "Python 3.11.8"
        v = _fs_parse_version(vtxt)
        if not v or v < (3, 10, 0):
            errors.append(f"Python >= 3.10.0 required. Found: {vtxt}")

    # --- pip or uv ---
    if not (shutil.which("pip") or shutil.which("uv")):
        errors.append("pip or uv not found. Install pip (usually with Python) or install uv.")

    # --- Frontend: Node + npm/pnpm ---
    if require_frontend:
        node = shutil.which("node")
        if not node:
            errors.append("Node.js not found. Install Node.js 18+ from https://nodejs.org/ (npm included).")
        else:
            vtxt = _fs_run([node, "-v"])  # e.g., "v18.19.0"
            v = _fs_parse_version(vtxt)
